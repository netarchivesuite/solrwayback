...
import dk.kb.netarchivesuite.solrwayback.parsers.ScriptRewriter;
...
public class SolrWaybackResource {
    ...
    @Path("/image")
    @Produces("image/png")
    public Response getImage(@QueryParam("source_file_path") String source_file_path, @QueryParam("offset") long offset, @QueryParam("height") int height,
            @QueryParam("width") int width) throws SolrWaybackServiceException {
        ...
        try {
            ...
            // log.debug("Getting image from source_file_path:" + source_file_path + "
            // offset:" + offset + " targetWidth:" + width + " targetHeight:" + height);

            ArcEntry arcEntry = Facade.getArcEntry(source_file_path, offset);

            // TODO: This is prone to OOM for large images. There should be a sanity check
            ...
            if (image == null) {
                ...
                throw new IllegalArgumentException("image is null and not .ico image, source_file_path:" + source_file_path + " offset:" + offset
                        + " contentType:" + arcEntry.getContentType());
            }
            ...
            throw handleServiceExceptions(e);
        } catch (Throwable e) { // Can happen due to the batik dependency. Sometimes SVG scaling failes. Also
                                // weird image formats can fail
            log.error("Throwable error in getImage:" + source_file_path + " offset:" + offset + " height:" + height + " width:" + width + " error:"
                    + e.getMessage()); // Java can not read all images.
    ...
    public Response downloadRaw(@QueryParam("source_file_path") String source_file_path, @QueryParam("offset") long offset) throws SolrWaybackServiceException {
        try {
            ...
            // JavaScript needs its `location` references rewritten so client-side navigation
            // (e.g. location.href = url) is caught and redirected back into playback instead of
            // leaking to the live web. This requires the full text in hand, so JS responses are
            // read and transformed rather than streamed byte-for-byte like other content types.
            String contentTypeForCheck = arcEntry.getContentType();
            if (contentTypeForCheck != null && contentTypeForCheck.toLowerCase().contains("javascript")) {
                return buildRewrittenJavaScriptResponse(arcEntry, source_file_path, offset);
            }

            InputStream in = arcEntry.getBinaryNoChunking();
    ...
    @Path("/export/warc")
    @Produces(MediaType.APPLICATION_OCTET_STREAM)
    public Response exportWarc(@QueryParam("query") String q, @QueryParam("fq") List<String> fq, @QueryParam("gzip") boolean gzip)
            throws SolrWaybackServiceException {
    ...
    @Produces(MediaType.APPLICATION_OCTET_STREAM)
    public Response exportWarcExpanded(@QueryParam("query") String q, @QueryParam("fq") List<String> fq, @QueryParam("gzip") boolean gzip)
            throws SolrWaybackServiceException {
        // This is also required even if the option is removed on the web-page.
        if (!PropertiesLoaderWeb.ALLOW_EXPORT_WARC) {
            throw new InvalidArgumentServiceException("Export to warc not allowed!");
    ...
    private Response exportWarcImpl(String q, List<String> fqList, boolean gzip, boolean expandResources, boolean avoidDuplicates)
            throws SolrWaybackServiceException {
        InputStream is = null;
        try {
     ...
     *             versions. Use '/export/fields' instead. Have frontend switch to
     *             this method.
     *
     */
    @Deprecated
    @GET
    @Path("/export/csv")
    public Response exportFull(@QueryParam("query") String q, @QueryParam("fields") String fields, @QueryParam("fq") String... filters)
            throws SolrWaybackServiceException {
        if (!PropertiesLoaderWeb.ALLOW_EXPORT_CSV) {
            throw new InvalidArgumentServiceException("Export to csv not allowed!");
        }
    ...
    @GET
    @Path("/export/fields")
    public Response exportFields(@QueryParam("query") String q, @QueryParam("fields") String fields, @QueryParam("expandResources") Boolean expandResources,
            @QueryParam("ensureUnique") Boolean ensureUnique, @QueryParam("groupfield") String groupField, @QueryParam("flatten") Boolean flatten,
            @QueryParam("format") String format, @QueryParam("gzip") Boolean gzip, @QueryParam("fq") String... filters) throws SolrWaybackServiceException {
        if (!PropertiesLoaderWeb.ALLOW_EXPORT_CSV) {
        ...
        try {
            log.debug("{} export. Query:'{}, filterquery:'{}', fields:'{}', expandResources:{}, ensureUnique:{}, flatten:{}, groupfield:{}, gzip:{}", format, q,
                    filters, fields, Boolean.TRUE.equals(expandResources), Boolean.TRUE.equals(ensureUnique), Boolean.TRUE.equals(flatten), groupField, gzip);
    ...
    /**
     * Endpoint that delivers a zip file of content present in query of a specific
     * content type.
     * 
    ...
    public Response exportZipContent(@QueryParam("query") String query, @QueryParam("fq") String... filters)
        ...
        return Response.ok(zip).header("Content-Disposition", getDisposition("solrwayback_$DATETIME.zip")).build();

    }

    /*
     * // * This will be called from solrwayback page views, when resources can not
     * be resolved (not harvested)
     */
    @GET
    @Path("/notfound")
...
//    log.debug("not found called");
    ...
    @GET
    @Path("/getContentType")
    public String getContentType(@QueryParam("source_file_path") String source_file_path, @QueryParam("offset") long offset)
            throws SolrWaybackServiceException {
        try {
    ...
    public Response waybackProxyAPIResolver(@Context UriInfo uriInfo, @PathParam("path") String path) throws SolrWaybackServiceException {
        try {
            ...
            // log.info("Found url with harvesttime:"+doc.getUrl() +" and
            // arc:"+doc.getArc_full());
    ...
    /*
     * Called from sw.js (serviceworker). It is called when a resource leaks to the
     * domain without the /solrwayback/ contextroot.
     * 
     * The referer url can be playback API (often HTML page) or warcfile+offs
     * (javascript or CSS)
     * 
     */
    @GET
    @Path("/webProxyLeak/{path:.+}")
    public Response webProxyLeak(@Context UriInfo uriInfo, @Context HttpServletRequest httpRequest, @PathParam("path") String path)
            throws SolrWaybackServiceException {
        try {
            // For some reason the var regexp does not work with comma (;) and other
            // characters. So I have to grab the full url from uriInfo
            ...
            // log.info("WebProxyLeak urldata:"+urlData);// example
            // https://solrwaybackserver:4000/sites/all/images/horse.png;
    ...
    @GET
    @Path("/web/{path:.+}")
    public Response waybackAPIResolver(@Context UriInfo uriInfo, @Context HttpServletRequest httpRequest, @PathParam("path") String path)
            throws SolrWaybackServiceException {
        return PathResolver.waybackAPIResolverHelper(this, "/web/", uriInfo, httpRequest, path, false);
    }

    /*
     * Playback with lenient URL resolving. The last part of the path '/web/' is the
     * same as wayback machine uses.
     *
     * JAX-RS syntax to match all after /lenient/web/.
     */
    @GET
    @Path("/lenient/web/{path:.+}")
    public Response waybackAPIResolverLenient(@Context UriInfo uriInfo, @Context HttpServletRequest httpRequest, @PathParam("path") String path)
            throws SolrWaybackServiceException {
    ...
    /*
     * will be called with
     * pwid/web/urn:pwid:netarkivet.dk:2018-12-10T06:27:01Z:part:https://www.
     * petdreams.dk/katteracer-siameser
     * 
     * Jersey syntax to match all after pwid/web/.
     */
    @GET
    @Path("/pwid/web/{path:.+}")
    public Response waybackPwidAPIResolver(@Context UriInfo uriInfo, @Context HttpServletRequest httpRequest, @PathParam("path") String path)
            throws SolrWaybackServiceException {
        try {
            // For some reason the var regexp does not work with comma (;) and other
            // characters. So I have to grab the full url from uriInfo
            ...
            if (!(urlCollectionName.equals(thisCollectionName))) {
                ...
                throw new InvalidArgumentServiceException(
                        "Wrong collection. This collection has PWID:" + thisCollectionName + " requested collection name was:" + urlCollectionName);
            }
    ...
    /*
     * This happens for leaks to solrwayback/services/ The proxy will handle it
     */
    @GET
    @Path("/{path:.+}")
    public Response waybackAPIResolverRoot(@Context UriInfo uriInfo, @Context HttpServletRequest httpRequest, @PathParam("path") String path)
            throws SolrWaybackServiceException {
        try {
            ...
            // Referer:
            // http://teg-desktop.sb.statsbiblioteket.dk:8080/solrwayback/services/view?source_file_path=/media/teg/1200GB_SSD/solrwayback_package_3.2_webrecorder/indexing/warcs/thomas_egense_dk.warc&offset=9485066
    ...
    public Response viewForward(@QueryParam("source_file_path") String source_file_path, @QueryParam("offset") long offset,
            @QueryParam("showToolbar") Boolean showToolbar, @QueryParam("lenient") Boolean lenient) throws SolrWaybackServiceException {
        try {
            ...
            String newUrl = Boolean.TRUE.equals(lenient) ? PropertiesLoader.WAYBACK_BASEURL + "services/lenient/web/" + waybackDate + "/" + url
            ...
            // String
            // newUrl="http://kb-test-way-001.kb.dk:8082/jsp/QueryUI/Redirect.jsp?url="+url+"&time="+waybackDate;
    ...
    public Response viewFromLeakedResource(@QueryParam("source_file_path") String source_file_path, @QueryParam("offset") long offset,
            @QueryParam("urlPart") String urlPart) throws SolrWaybackServiceException {
        // this method is only called from the tomcat solrwaybackrootproxy if that proxy
        // mode is used.
        try {
     ...
     * timestampsFrontEnd(@QueryParam("source_file_path") String
     * source_file_path, @QueryParam("offset") long offset) throws Exception {
     * return Facade.proxyBackendResources(source_file_path, ""+offset,
     * "timestampsforpage");
     * 
     * }
     */
    ...
    public Response viewImpl(String source_file_path, long offset, Boolean showToolbar, Boolean lenient) throws Exception {
        ...
        // log.debug("warc content charset:"+arcEntry.getContentCharset() +" warc
        // content type:"+arcEntry.getContentType());
        if (contentType == null) {
            // log.debug("no contenttype, using content_type from
            // tika:"+doc.getContentType());
            contentType = doc.getContentType();
        }
        if (arcEntry.getContentCharset() != null) {
            contentType = contentType + "; charset=" + arcEntry.getContentCharset();
        ...
        // charset="+arcEntry.getContentEncoding());
...
//          
    ...
    @GET
    @Path("/viewhref")
    public Response viewhref(@QueryParam("url") String url, @QueryParam("crawlDate") String crawlDate, @QueryParam("showToolbar") Boolean showToolbar)
            throws SolrWaybackServiceException {
        try {
    ...
    /*
     * Some leaks refers to the contextroot of the webapplication(/solrwayback).
     * Etc. /solrwayback/images/horse.png This page does not exist and the tomcat
     * error page will forward to this method. Both the original url and the
     * resource (warc+offset) are known, so the correct relative url can be
     * constructed.
     */
    ...
    /**
     * Calls {@link #applyTemplate(String)} with the given template and uses the
     * result to construct a HTTP Content-Disposition for downloading.
     * {@code getDisposition("solrwayback_$DATETIME.warc")} might return
     * {@code attachment; filename="solrwayback_2022-10-14_23-36-04.warc"}.
     * 
     * @param template a template to expand. {code $DATETIME} will be replaced with
     *                 a timestamp.
    ...
    /**
     * Apply the given template. Currently only {@code $DATETIME} will be expanded
     * to the current datetime: {@code applyTemplate("solrwayback_$DATETIME.warc")}
     * might return {@code solrwayback_2022-10-14_23-36-04.warc}.
     * 
     * @param template a template to expand. {code $DATETIME} will be replaced with
     *                 a timestamp.
    ...
    /**
     * Reads a JavaScript ArcEntry in full, rewrites {@code location} references so client-side
     * navigation is caught by the wayback toolbar's {@code _WB_wombat_location} shim instead of
     * leaking to the live web, and returns the rewritten bytes as the response.
     * <p>
     * Unlike the generic streaming path, this necessarily buffers the full content in memory,
     * since regex rewriting requires the complete text.
     * @param arcEntry the JavaScript resource to rewrite.
     * @param source_file_path used only for error logging.
     * @param offset used only for error logging.
     * @return the response with rewritten JavaScript content.
     */
    private Response buildRewrittenJavaScriptResponse(ArcEntry arcEntry, String source_file_path, long offset) throws IOException{
        String content = arcEntry.getStringContentAsStringSafe();
        String rewritten;
        try {
            rewritten = ScriptRewriter.rewriteLocationReferences(content);
        } catch (Exception e) {
            log.warn("Exception while rewriting external JavaScript for " + source_file_path + " offset:" + offset, e);
            rewritten = content; // fail open: serve original content rather than fail the request
        }
        ...
        ResponseBuilder response = Response.ok(rewritten).type(contentType);

        if (arcEntry.getFileName() != null) {
            response.header("Content-Disposition", "filename=\"" + arcEntry.getFileName() + "\"");
        }
        // Content-Encoding intentionally omitted: rewritten is a decoded String served fresh,
        // so any original compression encoding (gzip etc.) no longer applies to this response body.
